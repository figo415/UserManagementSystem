...
namespace Megarobo.KunPengLIMS.Infrastructure.Repositories
{
    ...
    /// 用户管理仓储实现
    /// </summary>
    public class UserRepository : LimsRepositoryBase<User>, IUserRepository
    {
        public UserRepository(LimsDbContext dbcontext) : base(dbcontext)
        {
        ...
        public User GetWithRoles(Guid id)
        {
            ...
            if (user != null)
            {
                //user.Roles = _dbContext.Set<UserRole>().Where(it => it.UserID == id).ToList();
            }
            return user;
        }
    }
